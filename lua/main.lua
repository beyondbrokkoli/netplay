...
local function main()
    print("========================================")
    print(" WEAVER ENGINE: HYBRID WAN/LAN CLIENT   ")
    print("========================================")

    -- 1. Execute Network Handshake FIRST
    ...
    -- 3. Boot Vulkan Graphics Engine
    print("[LUA IO] Booting Weaver (GRAPHICS)...")
    ...
    print("[NET] Scene loaded. Camera unlocked. Entering Networked 3-Phase Loop...")

    while ffi.C.vx_core_is_running() == 1 do

        -- (1) WINDOW RESIZE SAFETY GUARD
        ...
        if is_resizing then
            ...
            -- Pull everything off the wire before we do any local math
            Pump.intercept_network(ctx, ctx.sim_tick_count)


            -- ASYNC INPUT GATHERING
            -- Raycast the mouse strictly outside the fixed-timestep loop
            ...
            -- PHASE 2: DETERMINISTIC SIMULATION (Domain A)
            ...
            while ctx.accumulator >= FIXED_DT do
                ...
                -- Purge the slot if we've wrapped around the ring buffer
                ...
                -- Inject the local action we captured above
                if pending_click ~= -1 then
                    EngineSubmitCommand(ctx, 1, 0, 0, pending_click)
                    pending_click = -1 -- Consume it immediately so it doesn't double-fire
                end
            ...
            -- Acquire a hardware stream slot and fire
            local write_idx = ffi.C.vx_stream_acquire()
            if write_idx ~= -1 then
                -- Explicit Pointer Handoff: CPU Grid -> VRAM Mapper
