...
local function boot_weaver()
    ...
    for i, stage in ipairs(seq.boot) do
        print(string.format("[+] Executing Stage %d: %s", i, stage.name))
        local signal = stage.action(ctx)

        if signal == "AWAIT_SURFACE" then
            print("[+] Yielding execution, waiting for C-Core Surface...")
            local boot_start_time = get_time_hires()
            local TIMEOUT_SECONDS = 5.0

            while ffi.C.vx_sys_get_surface() == nil do
                if (get_time_hires() - boot_start_time) > TIMEOUT_SECONDS then
                    error(string.format(
                        "\n[!] FATAL: Surface initialization timed out after %.1f seconds.\n" ..
                        "[!] Are you running a graphical client in a headless environment?", 
                        TIMEOUT_SECONDS
                    ))
                end
                sys_sleep(10)
            ...
            print("[+] C-Core Surface acquired successfully.")
        end
    end
    return ctx
end
...
local function main()
    ...
    for p = 0, cfg_net.MAX_PLAYERS - 1 do
        ...
        if active_peers[p] then ctx.peer_active[p] = true else ctx.peer_active[p] = false end
    end
    ...
    print("[LUA IO] Booting Weaver Graphical Chassis...")
    ...
    print("[NET] Scene loaded. Camera unlocked. Entering 3-Phase Main Loop...")
        ...
        if is_resizing then
            if (get_time_hires() - last_resize_time) > RESIZE_COOLDOWN then
                ...
                if new_w[0] > 0 and new_h[0] > 0 then
                    ...
                    gfx_pipeline_module.Destroy(vk_rt.vk, vk_rt, gfx)
                    require("renderer").Destroy(vk_rt.vk, vk_rt.device, sync, cfg.cfg.frame_slots)

                    cfg.win.w = new_w[0]
                    cfg.win.h = new_h[0]

                    local mini_ctx = { vk_runtime = vk_rt, desc_state = desc, old_swapchain = sc.handle }
                    local resize_co = coroutine.create(function()
                        for _, stage in ipairs(seq.resize) do stage.action(mini_ctx) end
                        return mini_ctx
                    end)
            ...
            -- [+] PHASE 1: NETWORK DRAIN
            Pump.intercept_network(ctx, ctx.sim_tick_count)

            if not palette_ready and palette_job_id ~= -1 then
                if memory.IsTransferComplete(vk_rt, palette_job_id) then palette_ready = true end
            end

            -- [+] PHASE 2: THE DETERMINISTIC TEMPORAL ENGINE
            ...
            while ctx.accumulator >= FIXED_DT do
                ...
                if mouse_left == 1 and prev_mouse_left == 0 then
                    ...
                    pending_click = matrix_raycast_terrain(
                ...
                if pending_click ~= -1 then
                    EngineSubmitCommand(ctx, 1, 0, 0, pending_click)
                end
            ...
            -- [+] PHASE 3: ASYNCHRONOUS RENDER PHASE
            local alpha = ctx.accumulator / FIXED_DT
            total_time = total_time + frame_time
            pc.total_time = total_time
            pc.dt = alpha

            camera_mod.update(cam, frame_time, ffi.C.vx_input_mouse_x(), ffi.C.vx_input_mouse_y(), sc.extent.width, sc.extent.height)
            camera_mod.get_matrices(cam, sc.extent.width, sc.extent.height, pc.viewProj, inv_vp)

            local write_idx = ffi.C.vx_stream_acquire()
            if write_idx ~= -1 then
                render_queue.PackFrame(write_idx, pc, ctx.rts_grid, ctx.snapshot_ring, alpha, vram_template, render_queues, active_render_mode, master_ptr, memory, gfx, desc, sc, ctx.total_tiles)
                ...
                if wants_hotswap then
                    gfx_pipeline_module.HotReloadShaders(vk_rt.vk, vk_rt, gfx, frame_count)
                    wants_hotswap = false
                end
