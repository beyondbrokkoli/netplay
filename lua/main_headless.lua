...
local function get_local_ip()
    local cmd = "ip route get 1.1.1.1 | awk '{for(i=1;i<=NF;i++) if($i==\"src\") print $(i+1)}'"
    local f = io.popen(cmd)
...
local function extract_true_64bit_token(json_string)
    ...
    for i = 1, #token_digits do
        ...
        if byte >= 48 and byte <= 57 then val = (val * 10) + (byte - 48) else break end
    end
    return val
end
...
local function main()
    ...
    print("[SYSTEM] Drop-in complete. Entering Headless FSM loop.")
        ...
        while ctx.accumulator >= FIXED_DT do
            ...
            if ctx.sim_tick_count % 120 == (ctx.net_identity * 10) then
                if ctx.last_bot_tick ~= ctx.sim_tick_count then
                    EngineSubmitCommand(ctx, 1, 0, 0, math.random(0, ctx.total_tiles - 1))
